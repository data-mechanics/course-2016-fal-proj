...
class distances(dml.Algorithm):
    ...
    def execute(trial = False):
        ...
        ###Faster way?
        commIndicatorsList = []
        for comIndicator in repo[distances.reads[0]].find():#Longitude, Latitude, Indicator Weight
        ...
        for r in range(len(Y)):
            #Y[r] should be a vector corresponding to the GPS distances for each community indicator
            #commIndicatorsList[r][2] is the Indicator's weight in the original tuple
            #newY.append(Y[r]*commIndicatorsList[r][2])#Then multiply each indicator's distances to all cell-centers by that indicator's weight
            
            newY.append(Y[r]*commIndicatorsList[r][2]/gpsDifference_1000Feet)#Scaling in terms of feet!#ohhh -- imagine a very small distance between 2 GPS coordinates * small weight if negative indicator --> then do the inverse of that
            
        newY = np.array(newY).T #Then, we transpose the matrix, so that it's cell-centers on the rows, and indicators as the columns

        #Then, we sum up (all the inverse (distance*weight) to every single community indicator) for a given cell-center
        inverseDistances = [sum(1/newY[v]) for v in range(len(newY))]#--each v is a row-vector, then we take the inverse distance of the entire vector to produce a new vector, then we sum up over that vector
        comScores = [(coordinates[y], inverseDistances[y]) for y in range(len(inverseDistances))]
        for x in comScores:
            entry = {}
            #-0.5503632372502951 for crime, -0.45764425804185416 property -- unscaled in terms of feet, with all the entertainment licenses, w/o libraries, using crime-frequency-in-a-box,
            #           ultimately, with a range of -40729834.70692352 to -3271536.568326791 for community score
            #...and actually, the correlation is the same if it's scaled to feet anyways -- with just a range of -114043.53717938546 to -9160.302391315001 instead
            entry.update({"cell_community_value": x[1],
            #Oh, shit... Was this what made everything shoot up? -- was originally 0.47841092002897156 crime, 0.40686143854224194 property -- unscaled in terms of feet, with all the entertainment licenses, using log(property value),
