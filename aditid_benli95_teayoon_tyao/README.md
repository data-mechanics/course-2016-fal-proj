...
To run the interactive visualization navigate into the project#3/histogram folder then run `$ python3 -m http.server` and open a webbrowser to localhost:8000. If you want to display it at another port you can add the number after the command: `$ python3 -m http.server 9000` will display the visualization on port 9000. This visualization displays the number of establishments with a certain number of drug crimes within the 2 mile radius and the number of normalized all crimes within the 2 mile radius.
