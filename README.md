# Narrative
We chose to use the following public datasets from the Boston Data Portal: Active Food Establishment Licenses/Map (outlines the locations of all restaurants in Boston), Summer Farmers' Market/Winter Farmers' Market (outlines the locations and availabilities of all farmers' markets in Boston in Summer and Winter), Corner Stores (outlines the locations of convenience stores and small markets in Boston), and Park Boston Monthly Transactions by Zones (outlines the number of transactions by month in certain parking zones in Boston). These datasets contain location data that need to be standardized. 
