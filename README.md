# Narrative
We chose to use the following public datasets from the Boston Data Portal: Active Food Establishment Licenses/Map (outlines the locations of all restaurants in Boston), Summer Farmers' Market/Winter Farmers' Market (outlines the locations and availabilities of all farmers' markets in Boston in Summer and Winter), and Corner Stores (outlines the locations of convenience stores and small markets in Boston). We will also be using the public dataset from the Boston Maps Open Data: Parking Meters (provide all locations via longitude and latitude of parking meters in Boston). These datasets contain location data that need to be standardized. 
