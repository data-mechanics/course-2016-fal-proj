...
class retrieve(dml.Algorithm):
    ...
    reads = []
    writes = ['ckarjadi_johnnyg7.p_Val', 'ckarjadi_johnnyg7.f_Pan','ckarjadi_johnnyg7.Active_Work_Zones']
    writes +=['ckarjadi_johnnyg7.Waze_Jams', 'ckarjadi_johnnyg7.f_Estab','ckarjadi_johnnyg7.c_Gard.']
    ...
    def execute(trial = False):
        ...
        if trial == False:
            url ='https://data.cityofboston.gov/resource/g5b5-xrwi.json?$limit=170000&$select=zipcode,gross_tax'
            ...
            url ='https://data.cityofboston.gov/resource/g5b5-xrwi.json?$limit=1000&$select=zipcode,gross_tax'
            ...
            url='https://data.cityofboston.gov/resource/fdxy-gydq.json?$select=businessname,zip'
            ...
            url = 'https://data.cityofboston.gov/resource/29yf-ye7n.json?$select=district,lat,long&$limit=1000'
            ...
            url = 'https://data.cityofboston.gov/resource/pvhv-55ac.json?$select=locx,locy&$limit=1000'
            response = urllib.request.urlopen(url).read().decode("utf-8")
            r = json.loads(response)
            s = json.dumps(r, sort_keys=True, indent=2)
    ...
    def provenance(doc = prov.model.ProvDocument(), startTime = None, endTime = None):
        ...
        resource_p_Val = doc.entity('bdp:n7za-nsjh', {'prov:label':'Property Values 2016', prov.model.PROV_TYPE:'ont:DataResource', 'ont:Extension':'json'})
        ...
        resource_h_Loc = doc.entity('bdp:rdqf-ter7', {'prov:label':'Hospitals', prov.model.PROV_TYPE:'ont:DataResource', 'ont:Extension:':'json'})
        resource_c_Inc = doc.entity('bdp:rdqf-ter7', {'prov:label':'Crime Occurences', prov.model.PROV_TYPE:'ont:DataResource', 'ont:Extension:':'json'})
        resource_w_Jams = doc.entity('bdp:rdqf-ter7', {'prov:label':'Traffic Jams', prov.model.PROV_TYPE:'ont:DataResource', 'ont:Extension:':'json'})
        ...
        doc.usage(p_Val, resource_p_Val, startTime, None,
                {prov.model.PROV_TYPE:'ont:Retrieval',
                 'ont:Query':'$limit=170000&$select=zipcode,gross_tax'
                }
            )
