...
class optimizeBusStops(dml.Algorithm):
    ...
    def map1D(ps):
        ...
        def unmap1D(qs):
            return optimizeBusStops._unmap1D(qs, ps)
    ...
    def findStops(numStops, points2D):
        ...
        points1D, unmap1D = optimizeBusStops.map1D(points2D)
        ...
        new_stops = [stop.tup() for stop in unmap1D(mapped_stops)]
        return new_stops

    def visualize_route(route,points_dict,intersecting_points,tree_keys,using_idx=True):
    ...
    def visualize_route_stops(route_dict, stops_dict, new_stops, intersecting_points, tree_keys, points_dict):
        '''Create visualizations for the route showing old and new stops'''
        ...
        os = mpatches.Patch(color='#ffc091', label='Original Stop locations')
        ns = mpatches.Patch(color='#ff6e00', label='Kmeans derived locations')

        plt.legend(handles=[os, ns], fancybox=True, loc=4, fontsize=20)
        ...
        plt.plot(route_x, route_y, color='#999999', linestyle='-', linewidth=11, zorder=3)
        ...
        plt.savefig('KmeanStops.png')

        plt.show()

    @staticmethod
    def execute(route_id = 39300, r=0.01, visual = False):
        ...
        intersecting_points = optimizeBusStops.routeTreeIntersection(route, tree, tree_keys, points_dict, r)
        proj = optimizeBusStops.projectPoints(route, points_dict, intersecting_points, tree_keys)

        # run k-means to find stops
        num_stops = len(route['properties']['route_stops'])
        new_stops = optimizeBusStops.findStops(num_stops, proj)

        # run visualization if true
        if visual:
            optimizeBusStops.visualize_route_stops(route, stops_dict, new_stops, intersecting_points, tree_keys, points_dict)

        #return routes_dict, stops_dict, points_dict, tree, tree_keys, intersecting_points, proj, new_stops
